...
def get_gaze_features(word_sequence, position, gaze_dictionary):
    """
    ...
    if word in gaze_dictionary:
        word_gaze = gaze_dictionary[word]
    else:
        word_gaze = gaze_dictionary[UNKNOWN_SYMBOL]
    for i, value in enumerate(word_gaze):
        features["gaze(0)_at({0})".format(i + 1)] = value
    ...
    if position > 0:
        ...
        if word in gaze_dictionary:
            ...
            word_gaze = gaze_dictionary[UNKNOWN_SYMBOL]
        for i, value in enumerate(word_gaze):
            features["gaze(-1)_at({0})".format(i + 1)] = value
...
class SequenceDataFeatureExtractor(object):
    ...
    def __get_features(self, observation_sequence, position):
        ...
        elif self.feature_template == "gaze":
            assert self.__word_gaze is not None
            raw_features = get_gaze_features(observation_sequence,
                                                  position,
                                                  self.__word_gaze)
    ...
    def load_word_gazes(self, gaze_path):
        """Loads word gazes from a file in the given path."""
        self.__word_gaze = {}
        with open(gaze_path, "r") as infile:
            for line in infile:
                toks = line.split()
                if len(toks) == 0:
                    continue
                ...
                self.__word_gaze[toks[1]] = \
                    numpy.array([float(tok) for tok in toks[2:]])

                # Always normalize word gazes. <-- do we want to do this? try with and without
                self.__word_gaze[toks[1]] /= \
                    numpy.linalg.norm(self.__word_gaze[toks[1]])

        # Assert that the token for unknown word types is present.
        assert UNKNOWN_SYMBOL in self.__word_gaze

        # Address some treebank token conventions.
        if "(" in self.__word_gaze:
            self.__word_gaze["-LCB-"] = self.__word_gaze["("]
            self.__word_gaze["-LRB-"] = self.__word_gaze["("]
            self.__word_gaze["*LCB*"] = self.__word_gaze["("]
            self.__word_gaze["*LRB*"] = self.__word_gaze["("]
        if ")" in self.__word_gaze:
            self.__word_gaze["-RCB-"] = self.__word_gaze[")"]
            self.__word_gaze["-RRB-"] = self.__word_gaze[")"]
            self.__word_gaze["*RCB*"] = self.__word_gaze[")"]
            self.__word_gaze["*RRB*"] = self.__word_gaze[")"]
        if "\"" in self.__word_gaze:
            self.__word_gaze["``"] = self.__word_gaze["\""]
            self.__word_gaze["''"] = self.__word_gaze["\""]
            self.__word_gaze["`"] = self.__word_gaze["\""]
            self.__word_gaze["'"] = self.__word_gaze["\""]
...
def main(args):
    ...
    if args.train:
        ...
        if args.gaze_path:
            feature_extractor.load_word_gazes(args.gaze_path)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--feature_template",
                           ...
                           " (default: %(default)s)")
    argparser.add_argument("--gaze_path", type=str, help="path to word "
                           "gazes")
